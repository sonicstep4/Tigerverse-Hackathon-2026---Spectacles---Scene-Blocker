...
class Snap3D {
    ...
    static async repeatGetStatus(submitGetStatus) {
        ...
        while (!taskCompleted) {
            try {
                const response = await Promisfy_1.Promisfy.RemoteServiceModule.performApiRequest(RSM_GETSTATUS, getStatusApiRequest);
                if (response.statusCode != 1) {
                    taskCompleted = true;
                ...
                if (responseBody.stage === "base_mesh_gen" &&
                    ...
                    for (const artifact of responseBody.artifacts) {
                        if (artifact.artifact_type === "image") {
                            ...
                            const dynamicImgResource = await Promisfy_1.Promisfy.InternetModule.performHttpRequest(INTERNET_MODULE, httpRequest);
                            let texture;
                            try {
                                texture =
                    ...
                    for (const artifact of responseBody.artifacts) {
                        if (artifact.artifact_type === "base_mesh") {
                            ...
                            const dynamicGltfResource = await Promisfy_1.Promisfy.InternetModule.performHttpRequest(INTERNET_MODULE, httpRequest);
                            try {
                                const gltfAsset = await Promisfy_1.Promisfy.RemoteMediaModule.loadResourceAsGltfAsset(REMOTE_MEDIA_MODULE, dynamicGltfResource.asResource());
     ...
     */
    static async submitImageTo3D(imageUrl, options) {
        const requestBody = {
            prompt: options?.prompt || "an object",
            format: "glb",
            refine: options?.refine !== undefined ? options.refine : true,
            use_vertex_color: options?.use_vertex_color !== undefined
