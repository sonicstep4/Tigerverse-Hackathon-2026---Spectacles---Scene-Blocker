...
import {StageCalibrationController} from "./StageCalibrationController"
...
export class SpeechToText extends BaseScriptComponent {
  ...
  @input
  @hint("First index containing real script lines (intro/start slide is before this)")
  firstContentSlideIndex: number = 1

  @input
  @hint("Voice phrase used to start from intro slide")
  startPhrase: string = "start"

  @input
  @allowUndefined
  @hint("Optional calibration controller called when start phrase is spoken")
  stageCalibrationController: StageCalibrationController

  @input
  @allowUndefined
  @hint("Optional SFX audio component played when advancing to next line")
  lineAdvanceSfx: AudioComponent
  ...
  private handleTranscription(transcription: string) {
    ...
    if (currentSlideIndex < this.firstContentSlideIndex) {
      ...
      const startMatched =
        normalizedText === normalizedStart ||
        normalizedText.includes(" " + normalizedStart) ||
        normalizedText.startsWith(normalizedStart + " ") ||
        normalizedText === "lets start" ||
        normalizedText === "let's start" ||
      ...
      if (startMatched) {
        ...
        if (this.presentationSwitcher) {
          this.presentationSwitcher.goToIndex(this.firstContentSlideIndex)
        } else {
  ...
  private playLineAdvanceSfx() {
    if (!this.lineAdvanceSfx) {
      return
    }

    const sfx: any = this.lineAdvanceSfx as any
    try {
