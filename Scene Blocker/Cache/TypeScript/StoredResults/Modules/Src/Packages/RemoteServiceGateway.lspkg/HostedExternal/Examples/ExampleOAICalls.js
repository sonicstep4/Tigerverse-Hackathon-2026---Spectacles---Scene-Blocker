...
let ExampleOAICalls = (() => {
    ...
    var ExampleOAICalls = _classThis = class extends _classSuper {
        ...
        onAwake() {
            // Initialize logger
            this.logger = new Logger_1.Logger("ExampleOAICalls", this.enableLogging || this.enableLoggingLifecycle, true);
        ...
        doImageGeneration() {
            ...
            OpenAI_1.OpenAI.imagesGenerate({
                ...
                if (url) {
                    ...
                    httpRequest.url = url;
                    return Promisfy_1.Promisfy.InternetModule.performHttpRequest(this.internetModule, httpRequest).then((httpResponse) => {
                        const resource = httpResponse.asResource();
                        return Promisfy_1.Promisfy.RemoteMediaModule.loadResourceAsImageTexture(this.rmm, resource);
                    });
                }
                else if (b64) {
                    print("Decoding texture from base64");
                    return new Promise((resolve, reject) => {
                        Base64.decodeTextureAsync(b64, (texture) => resolve(texture), () => reject("Failure to decode texture from base64"));
                    });
                }
            })
                .then((texture) => {
                ...
                OpenAI_1.OpenAI.imagesEdit(imageEditRequest)
                    ...
                    response.data.forEach(async (datum) => {
                        ...
                        if (url) {
                            ...
                            const httpRequest = RemoteServiceHttpRequest.create();
                            httpRequest.url = url;
                            const resource = await Promisfy_1.Promisfy.InternetModule.performHttpRequest(this.internetModule, httpRequest);
                            this.rmm.loadResourceAsImageTexture(resource, (texture) => {
                                const imgComponent = this.editResultObject.getComponent("Image");
                                const imageMaterial = imgComponent.mainMaterial.clone();
                                imgComponent.mainMaterial = imageMaterial;
                                imgComponent.mainPass.baseTex = texture;
                            }, () => {
