...
exports.AI_RESPONSE_POLICY = exports.CLEANING_STEPS = exports.TROUBLESHOOTING = exports.DESCALING_STEPS = exports.SAFETY_WARNINGS = exports.LIGHT_STATES = exports.MACHINE_INFO = void 0;
exports.formatGuidanceContextForPrompt = formatGuidanceContextForPrompt;
exports.buildSystemPromptContext = buildSystemPromptContext;
// ─── Machine Overview ────────────────────────────────────────────────────────
exports.MACHINE_INFO = {
...
exports.DESCALING_STEPS = [
    {
        ...
        assistantFocus: "Descaling prep only: official descaler packet + fresh water to half a tank, swirl to mix. Do not talk about ejecting a spent coffee capsule or closing the head after a brew—that is normal brewing, not this step.",
    },
    {
        id: 2,
        phase: "preparation",
 ...
 */
function formatGuidanceContextForPrompt(step) {
    const focus = step.assistantFocus != null && step.assistantFocus.length > 0 ? step.assistantFocus : step.description;
    let block = `[Active Nespresso Vertuo DESCALING guide – Step ${step.id}/${exports.DESCALING_STEPS.length}]\n` +
        `Phase: ${step.phase}\n` +
        `Title: ${step.title}\n` +
...
function buildSystemPromptContext() {
    ...
    ctx += `Machine: ${exports.MACHINE_INFO.name}\n`;
    ctx += `Technology: ${exports.MACHINE_INFO.technology}\n`;
    ctx += `Maintenance: ${exports.MACHINE_INFO.maintenanceSchedule}\n`;
    ctx += `Required for descaling: ${exports.MACHINE_INFO.requiredForDescaling.join(", ")}\n\n`;
    ctx += "BUTTON / LIGHT STATES:\n";
    exports.LIGHT_STATES.forEach((l) => {
        ctx += `  ${l.state}: ${l.meaning}\n`;
    });
    ctx += "\nSAFETY WARNINGS:\n";
    exports.SAFETY_WARNINGS.forEach((w) => {
        ctx += `  - ${w}\n`;
    });
    ctx += `\nDESCALING STEPS (${exports.DESCALING_STEPS.length} steps, ~20 min):\n`;
    exports.DESCALING_STEPS.forEach((step) => {
    ...
    ctx += "\nTROUBLESHOOTING:\n";
    exports.TROUBLESHOOTING.forEach((issue) => {
    ...
    exports.CLEANING_STEPS.forEach((step, i) => {
        ctx += `  ${i + 1}. ${step}\n`;
    });
    ctx += "=== END NESPRESSO KNOWLEDGE BASE ===\n";
    return ctx;
}
//# sourceMappingURL=NespressoKnowledge.js.map