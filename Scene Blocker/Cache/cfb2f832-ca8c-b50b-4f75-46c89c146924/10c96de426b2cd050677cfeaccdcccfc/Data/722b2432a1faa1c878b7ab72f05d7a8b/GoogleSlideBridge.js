...
let GoogleSlideBridge = (() => {
    ...
    var GoogleSlideBridge = _classThis = class extends _classSuper {
        ...
        async onAwake() {
            this.logger = new Logger_1.Logger("GoogleSlideBridge", this.enableLogging || this.enableLoggingLifecycle, true);
            if (this.enableLoggingLifecycle) {
                this.logger.debug("LIFECYCLE: onAwake() - Component initializing");
            }
            if (this.presentationId === "" || this.accessToken === "") {
                this.logger.warn("Token and id fields are incorrect or empty. Please set it in the inspector.");
                return;
            }
            ...
            if (!this.internetModule) {
                this.logger.error("Internet Module is not available.");
                return;
            }
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    this.logger.debug(`Fetch successful: ${JSON.stringify(data)}`);
                }
                else {
                    this.logger.warn(`Fetch failed with status: ${response.status}`);
                }
            }
            catch (error) {
                this.logger.error(`Error during fetch: ${error}`);
            }
            this.logger.debug(`Internet Module available: ${!!this.internetModule}`);
            this.logger.debug(`Internet Module available: ${!!this.remoteMediaModule}`);
            if (!global.deviceInfoSystem.isInternetAvailable()) {
                this.logger.error("Internet is not available. Cannot proceed with API requests.");
                return;
            }
        }
        initialize() {
            if (!this.accessToken || !this.presentationId) {
                this.logger.warn("Missing access token or presentation ID. Google Slides integration disabled.");
                return;
            }
        ...
        async logResponseError(response) {
            try {
                ...
                this.logger.warn(`API Error Response: ${errorText}`);
            }
            catch (e) {
        ...
        async validateCredentials() {
            try {
                ...
                if (response.status === 200) {
                    this.logger.debug("Google Slides API connection successful");
                    return true;
                }
                else {
                    this.logger.warn(`API validation failed with status: ${response.status}`);
                    return false;
                }
            }
            catch (error) {
                this.logger.error("Failed to validate Google credentials: " + error);
                return false;
            }
        }
        ...
        async fetchPresentationData() {
            try {
                if (!this.internetModule || !this.internetModule.fetch) {
                    this.logger.error("Internet Module or fetch method is not available.");
                    return;
                }
                ...
                if (response.status === 200) {
                    ...
                    this.logger.debug(`Loaded presentation with ${this.slideCount} slides`);
                    ...
                    if (slideData && slideData.slideProperties && slideData.slideProperties.notesPage) {
                        ...
                        if (notesPage.pageElements) {
                            ...
                            if (notesText.trim()) {
                                ...
                                this.logger.debug("Speaker notes loaded successfully");
                            }
                            else {
                                this.speakerNotes.text = "No speaker notes available";
                                this.logger.debug("No speaker notes content found");
                            }
                        }
                        else {
                            this.speakerNotes.text = "No speaker notes available";
                            this.logger.debug("Notes page has no elements");
                        }
                    }
                    else {
                        this.speakerNotes.text = "No speaker notes available";
                        this.logger.debug("No notes page found for this slide");
                    }
                }
                catch (error) {
                    this.speakerNotes.text = "Error loading speaker notes";
                    this.logger.error("Error processing speaker notes: " + error);
                }
                this.currentSlideIndex = index;
            }
            catch (error) {
                this.logger.error("Error loading slide content: " + error);
            }
        }
        // Load image from URL into the slideImage component
        loadImageFromUrl(url) {
            if (!this.slideImage) {
                this.logger.warn("SlideImage component is not defined.");
                return;
            }
            ...
            if (!this.remoteServiceModule || !this.remoteServiceModule.makeResourceFromUrl) {
                this.logger.error("RemoteServiceModule or makeResourceFromUrl is not available. Ensure you are testing on Spectacles or in Lens Studio with Device Type Override set to Spectacles.");
                return;
            }
            ...
            if (!this.remoteMediaModule || !this.remoteMediaModule.loadResourceAsImageTexture) {
                this.logger.error("RemoteMediaModule or loadResourceAsImageTexture is not available. Ensure you are testing on Spectacles or in Lens Studio with Device Type Override set to Spectacles.");
                return;
            }
            // Check internet availability
            if (!global.deviceInfoSystem.isInternetAvailable()) {
                this.logger.error("Internet is not available. Cannot load image.");
                return;
            }
            try {
                ...
                if (!resource) {
                    this.logger.error("Failed to create DynamicResource from URL.");
                    return;
                }
                ...
                this.remoteMediaModule.loadResourceAsImageTexture(resource, (texture) => {
                    if (texture) {
                        ...
                        this.logger.debug("Slide image loaded successfully.");
                    }
                    else {
                        this.logger.warn("Failed to load slide image texture.");
                    }
                }, (error) => {
                ...
                this.logger.error("Error in loadImageFromUrl: " + error);
            }
        }
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (remoteSlideCount !== this.slideCount) {
                        this.logger.debug("Remote slide count changed. Updating slides...");
                ...
                this.logger.error("Error checking for remote slide changes: " + error);
            }
        }
        ...
        async goToSlideInGoogle(index) {
            if (!this.hasValidCredentials || index < 0 || index >= this.slideCount) {
                this.logger.warn("Cannot go to slide. Either credentials are invalid or index is out of bounds.");
                return;
            }
            try {
                ...
                if (global.deviceInfoSystem.isInternetAvailable()) {
                    try {
                        ...
                        if (response.status === 200) {
                            this.logger.debug(`Successfully navigated to slide ${index + 1} in presentation mode`);
                            this.currentSlideIndex = index;
                        }
                        else {
                            this.logger.warn(`Failed to navigate to slide: ${response.status}`);
                        }
                    }
                    catch (error) {
                        this.logger.error("Error navigating to slide: " + error);
                    }
                }
                else {
                    this.logger.error("Internet is not available. Cannot navigate to slide.");
                }
            }
            catch (error) {
                this.logger.error("Error going to slide in Google Slides: " + error);
            }
        }
        // Go to next slide
        next() {
        ...
        async sendKeyboardShortcut(key) {
            if (!this.hasValidCredentials) {
                this.logger.warn("Cannot send keyboard shortcut. Invalid credentials.");
                return;
            }
            try {
                ...
                if (global.deviceInfoSystem.isInternetAvailable()) {
                    try {
                        ...
                        if (response.status === 200) {
                            this.logger.debug(`Successfully accessed slide ${this.currentSlideIndex + 1} in presentation mode`);
                        }
                        else {
                            this.logger.warn(`Failed to access slide: ${response.status}`);
                        }
                    }
                    catch (error) {
                        this.logger.error("Error accessing slide: " + error);
                    }
                }
                else {
                    this.logger.error("Internet is not available. Cannot access presentation.");
                    return;
                }
            }
            catch (error) {
                this.logger.error("Error sending keyboard shortcut: " + error);
            }
        }
        ...
        async moveToNextSlideInGoogle() {
            if (!this.hasValidCredentials || this.currentSlideIndex >= this.slideCount - 1) {
                this.logger.warn("Cannot move to the next slide. Either credentials are invalid or already on the last slide.");
                return;
            }
            try {
                ...
                this.logger.debug(`Successfully moved to the next slide (index ${nextSlideIndex}) in Google Slides.`);
            }
            catch (error) {
                this.logger.error("Error moving to the next slide in Google Slides: " + error);
            }
        }
        ...
        async moveToPreviousSlideInGoogle() {
            if (!this.hasValidCredentials || this.currentSlideIndex <= 0) {
                this.logger.warn("Cannot move to the previous slide. Either credentials are invalid or already on the first slide.");
                return;
            }
            try {
                ...
                this.logger.debug(`Successfully moved to the previous slide (index ${previousSlideIndex}) in Google Slides.`);
            }
            catch (error) {
                this.logger.error("Error moving to the previous slide in Google Slides: " + error);
            }
        }
    };
