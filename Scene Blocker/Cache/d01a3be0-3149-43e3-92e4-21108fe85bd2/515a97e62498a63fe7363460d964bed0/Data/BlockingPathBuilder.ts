export namespace BlockingPathBuilder {
  export function buildFromPoints(points: vec3[], width: number): RenderMesh | null {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const forward =
        i < points.length - 1 ? points[i + 1].sub(current) : current.sub(points[i - 1])
    ...
    for (let i = 0; i < segmentCount - 1; i++) {
      ...
      // Back face (reverse winding) for better visibility from all view angles
      indices.push(nextLeftIndex, rightIndex, leftIndex)
